...
{% block breadcrumbs %}
<a href="{% url 'dashboard' %}" class="hover:text-primary transition-colors" style="color: #a3a3a3; text-decoration: none;">Início</a> <span style="color: #a3a3a3;"> / </span> <a href="{% url 'product_list' %}" class="hover:text-primary transition-colors" style="color: #a3a3a3; text-decoration: none;">Produtos</a> <span style="color: #a3a3a3;"> / </span> <span style="font-weight: 600;">{% if is_edit %}Editar{% else %}Novo{% endif %}</span>
{% endblock %}
    ...
    <form method="post" class="card flex-col gap-6">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
            <div class="flex-col gap-2">
                ...
                <input type="text" name="name" value="{{ product.name }}" class="input" placeholder="Ex: Própolis 30ml" required>
            </div>

            <div class="flex-col gap-2">
                <label class="label">Código Único</label>
                {% if is_edit %}
                <input type="text" name="code" value="{{ product.code }}" class="input" placeholder="Ex: PRO30" required>
                {% else %}
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1.5rem;">
            <div class="flex-col gap-2">
                ...
                <input type="number" step="0.01" name="cost_price" value="{{ product.cost_price|stringformat:'.2f'|default:'0.00' }}" class="input" required>
            </div>

            <div class="flex-col gap-2">
                <label class="label">Preço de Venda (R$)</label>
                <input type="number" step="0.01" name="sale_price" value="{{ product.sale_price|stringformat:'.2f'|default:'0.00' }}" class="input" required>
            </div>

            <div class="flex-col gap-2">
                <label class="label">Estoque Mínimo</label>
                <input type="number" name="min_stock" value="{{ product.min_stock|default:'10' }}" class="input" required>
            </div>
        </div>
